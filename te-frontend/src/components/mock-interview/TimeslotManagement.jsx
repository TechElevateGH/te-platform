...
const TimeslotManagement = () => {
    ...
    const calculateEndTime = (startTime, interviewType) => {
        if (!startTime) return '';
        const [hours, minutes] = startTime.split(':').map(Number);
        const duration = INTERVIEW_TYPES[interviewType]?.duration || 55;
    ...
    const handleCreateBulk = async (e) => {
        ...
        try {
            ...
            await axiosInstance.post('/mock-interviews/timeslots/bulk', { timeslots: slotsToCreate }, {
                headers: { Authorization: `Bearer ${accessToken}` }
            });
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-3 gap-3">
                                                {Object.entries(INTERVIEW_TYPES).map(([key, type]) => {
                                                    ...
                                                    const isSelected = selectedInterviewType === key;
                                                    return (
                                                        <button
                                                            key={key}
                                                            type="button"
                                                        ...
                                                        >
                                                            <div className="flex flex-col items-center gap-2">
                                                                ...
                                                                <div className="text-center">
                                                                    ...
                                                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{type.duration} min</p>
                                                                </div>
                                                            </div>
                                                        </button>
                                                    );
                                                    ...
                                                    <div className="flex items-start gap-3 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
                                                        ...
                                                        <div className="text-sm">
                                                            <p className="font-semibold text-blue-900 dark:text-blue-200 mb-1">
                                                                {formData.start_time} - {calculateEndTime(formData.start_time, selectedInterviewType)}
                                                                <span className="font-normal text-blue-700 dark:text-blue-300"> ({INTERVIEW_TYPES[selectedInterviewType].duration} min)</span>
                                                            </p>
                                                            {slotCount > 1 && (
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-7 gap-1 mb-2">
                        {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
                            <div key={i} className="text-center text-xs font-semibold text-gray-400 dark:text-gray-500 py-1.5">
                                {day}
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-7 gap-1">
                        {calendarDays.map((day, idx) => {
                            ...
                            return (
                                <div
                                    key={idx}
                                    className={`aspect-square rounded-lg flex flex-col items-center justify-center text-xs transition-all relative ${isToday ? 'ring-2 ring-blue-500 ring-offset-1 dark:ring-offset-gray-800' : ''
                                        } ${isPast ? 'opacity-40' : ''} ${hasSlots
                                            ? availableCount > 0
                                                ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 font-semibold hover:bg-emerald-200 dark:hover:bg-emerald-900/50'
                                                : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 font-semibold hover:bg-amber-200 dark:hover:bg-amber-900/50'
                                            : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'
                                        }`}
                                >
                                    <span className="font-medium">{day.getDate()}</span>
                                    {hasSlots && (
                                        <span className="text-[9px] leading-none">{slots.length}</span>
                                    )}
                                </div>
                            );
                        })}
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-center gap-6 text-xs">
                                ...
                                >
                                    ...
                                    <div className="p-3 flex flex-wrap gap-2">
                                        {slots.map((slot) => (
                                            <div
                                                key={slot.id}
                                                className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${slot.is_available
