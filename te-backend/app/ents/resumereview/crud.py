...
def create_review_request(
    db: Database, *, user_id: str, user_name: str, user_email: str, data: review_schema.ResumeReviewCreate
...
def read_user_review_requests(db: Database, *, user_id: str) -> list[dict]:
    ...
    reviews_data = db.resume_reviews.find({"user_id": ObjectId(user_id)}).sort("submitted_date", -1)
    result = []
    for review in reviews_data:
        result.append({
            "id": str(review["_id"]),
...
def update_review_request(
    db: Database, *, review_id: str, reviewer_id: str, reviewer_name: str, data: review_schema.ResumeReviewUpdate
) -> dict:
    ...
    return {
        ...
        "reviewed_by": str(updated_review["reviewed_by"]) if updated_review.get("reviewed_by") else None,
