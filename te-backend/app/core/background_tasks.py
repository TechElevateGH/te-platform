...
async def send_meeting_reminders():
    ...
    try:
        ...
        logger.info(f"Checking for meetings between {reminder_time_start} and {reminder_time_end}")
        ...
        for meeting_data in meetings:
            try:
                ...
                meeting_datetime_str = f"{meeting.timeslot_date} {meeting.timeslot_time}"
                ...
                if reminder_time_start <= meeting_datetime <= reminder_time_end:
                    logger.info(f"Sending reminder for meeting {meeting.id} at {meeting_datetime}")
                    
                    # Get the assigned volunteer+ user
                    volunteer = None
                    if meeting.assigned_to:
                        volunteer_data = db.privileged_users.find_one({"_id": meeting.assigned_to})
                        if volunteer_data:
                    ...
                    if volunteer and volunteer.email:
                        try:
                            ...
                            logger.error(f"Failed to send reminder to volunteer {volunteer.email}: {e}")
                    ...
                    try:
                        ...
                        logger.error(f"Failed to send reminder to info@techelevate.org: {e}")
        ...
        logger.info(f"Meeting reminder task completed. Sent {reminders_sent} reminders.")
